...
Version 1.6.4 provides the shared Instagram and Facebook Local Companion engines. Facebook Most Viewed collection starts directly on the public Reels grid, then verifies every returned Reel against its own public page for the correct publish timestamp and exact current reactions/comments. Comment bodies are no longer collected or displayed. Public Facebook collection runs in a fresh local Chrome or Edge session for reliable profile hydration, with a fresh anonymous embedded browser as the browser-launch fallback.
