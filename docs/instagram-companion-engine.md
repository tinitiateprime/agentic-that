...
The website requires AgenticThat Companion `1.7.5` and extension `1.2.0` or newer for the current Companion contract. Companion `1.7.5` validates timezone-aware date, month, and year ranges, scans toward the requested lower boundary, and reports partial discovery when range coverage cannot be proven. Companion accepts a self-hosted dashboard's public verification key only when the request arrives through the trusted extension or desktop proxy; private signing keys remain on the website server.
