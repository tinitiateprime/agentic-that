...
function connectionString() {
  const url = (runtimeEnv(["DATABASE", "URL"]) || runtimeEnv(["SUPABASE", "DB", "URL"])).trim();
  if (!url) {
    throw new Error(
      "DATABASE_URL is not set. Paste your Supabase connection string from Project Settings > Database > Connection string."
    );
  }
