...
function privateKey() {
  ...
  if (configured) return crypto.createPrivateKey(configured);
...
function publicKeys() {
  ...
  const key = configured ? crypto.createPublicKey(configured) : crypto.createPublicKey(privateKey());
...
export function signServiceAccessToken({ audience, subject, workspaceId, grants, name = "", email = "" }, ttlSeconds = TOKEN_TTL_SECONDS) {
...
export function serviceTokenPublicKeyPem() {
  return crypto.createPublicKey(privateKey()).export({ format: "pem", type: "spki" }).toString();
}
