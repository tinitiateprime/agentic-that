...
function explicitPublicKeys(verifier) {
  const configured = envKey(verifier?.publicKey);
  const keyId = String(verifier?.keyId || "").trim();
  if (!configured || !keyId) return null;
  return new Map([[keyId, importPublicKey(configured)]]);
}
...
export function verifyServiceAccessToken(token, expectedAudience, verifier = null) {
  try {
    ...
    const keys = verifier ? explicitPublicKeys(verifier) : publicKeys();
    ...
    const issuer = verifier
      ? String(verifier.issuer || "").trim()
      : process.env.SERVICE_TOKEN_ISSUER?.trim() || "agenticthat";
    if (
      payload.iss !== issuer ||
