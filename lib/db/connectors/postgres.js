...
function connectionString() {
  const netlifyUrl = process.env.NETLIFY === "true" ? netlifyDatabaseUrl() : "";
  const url = (netlifyUrl || process.env.DATABASE_URL || process.env.SUPABASE_DB_URL || "").trim();
  if (!url) {
    throw new Error(
...
export function createPostgresConnector() {
  ...
  let ready;
  const ensureReady = () => (ready ||= (isNetlifyDatabaseUrl(url) ? bootstrapAdmin(sql) : migrate(sql).then(() => bootstrapAdmin(sql))));
...
function isNetlifyDatabaseUrl(url) {
  try {
    return new URL(url).hostname.endsWith(".db.netlify.com");
  } catch {
    return false;
  }
}
