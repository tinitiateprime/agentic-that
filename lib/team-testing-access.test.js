...
test("team testing access is disabled by default and requires an explicit true value", () => {
  const previous = process.env.NEXT_PUBLIC_TEAM_TESTING_FULL_ACCESS;
  try {
    delete process.env.NEXT_PUBLIC_TEAM_TESTING_FULL_ACCESS;
    assert.equal(teamTestingFullAccessEnabled(), false);
    assert.equal(teamTestingFullAccessEnabled(""), false);
    assert.equal(teamTestingFullAccessEnabled("true"), true);
    assert.equal(teamTestingFullAccessEnabled("1"), true);
  } finally {
...
test("team testing access stays disabled for common false values", () => {
