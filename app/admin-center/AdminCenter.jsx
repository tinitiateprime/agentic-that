...
import AdminWebsiteStudio from "./AdminWebsiteStudio";
...
function UserEditor({ user, roles, workspaces, onSaved }) {
  ...
  return <article className={`admin-user-card status-${user.status}`}>
    ...
    </div><h4>Workspace access</h4><p><span className="status-pill">{user.fullWorkspaceAccess ? "Full access" : user.billingStatus}</span></p><div className="role-checks">{(user.entitlements || []).map((entitlement) => <span key={`${entitlement.roleId}:${entitlement.source}`}><strong>{roles.find((role) => role.id === entitlement.roleId)?.name || entitlement.roleId}</strong> · {entitlement.source} · {entitlement.status}{entitlement.expiresAt ? ` · expires ${new Date(entitlement.expiresAt).toLocaleString()}` : ""}</span>)}</div><small>Active workspace owners unlock every service without a trial clock or trial usage quota. Employee permissions still follow assigned roles.</small>{error && <p className="admin-error">{error}</p>}<div className="admin-actions"><button disabled={busy} onClick={() => save(status)}>{busy ? "Saving…" : user.status === "pending" ? "Activate account" : "Save account"}</button>{user.status !== "pending" && <button disabled={busy} onClick={() => save(status, true)}>Revoke sessions</button>}{user.status === "pending" && <button className="danger" disabled={busy} onClick={() => save("rejected")}>Reject</button>}</div></div>}
  </article>;
}
...
export default function AdminCenter({ initialData, principal }) {
  ...
  return <main className={tab === "project-management" ? "admin-center-project-shell" : "admin-center-shell"}><aside className="admin-sidebar"><a href="/apps" className="admin-brand"><span>AT</span>AgenticThat</a><p>Global Admin Center</p>{[["users", `Users${pending ? ` (${pending})` : ""}`], ["website-studio", "AI Website Studio"], ["project-management", "Project management"], ["publishing", "Publishing monitor"], ["email-studio", "Email Studio"], ["roles", "Roles & permissions"], ["workspaces", "Workspaces"], ["reviews", `Identity reviews${reviewCount ? ` (${reviewCount})` : ""}`], ["audit", "Audit history"]].map(([id, label]) => <button className={tab === id ? "active" : ""} onClick={() => setTab(id)} key={id}>{label}</button>)}<div className="admin-identity"><strong>{principal.name}</strong><small>{principal.email}</small></div></aside>
    <section className={`admin-main${tab === "email-studio" ? " email-studio-main" : tab === "publishing" ? " monitor-main" : tab === "project-management" ? " project-management-main" : tab === "website-studio" ? " website-studio-main" : ""}`}>{!["email-studio", "publishing", "project-management", "website-studio"].includes(tab) && <header><p>Centralized access control</p><h1>{tab === "roles" ? "Roles and permissions" : tab.charAt(0).toUpperCase() + tab.slice(1)}</h1></header>}
      ...
      {tab === "website-studio" && <AdminWebsiteStudio />}
