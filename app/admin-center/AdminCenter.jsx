  ...
  return <article className="role-card">
    ...
    {expanded && <div className="admin-user-editor">
      {role.isSystem ? <p>System roles are read-only.</p> : <>
        ...
        <label><input type="checkbox" checked={draft.isSelfSelectable} onChange={(event) => setDraft({ ...draft, isSelfSelectable: event.target.checked })} /> Available for self-service signup and billing</label>
...
function UserEditor({ user, roles, workspaces, onSaved }) {
  ...
  return <article className={`admin-user-card status-${user.status}`}>
    ...
    </div><h4>Billing-controlled role entitlements</h4><p><span className="status-pill">{user.billingStatus}</span>{user.trialEndsAt && ` · Trial ends ${new Date(user.trialEndsAt).toLocaleString()}`}</p><div className="role-checks">{(user.entitlements || []).map((entitlement) => <span key={`${entitlement.roleId}:${entitlement.source}`}><strong>{roles.find((role) => role.id === entitlement.roleId)?.name || entitlement.roleId}</strong> · {entitlement.source} · {entitlement.status}{entitlement.expiresAt ? ` · expires ${new Date(entitlement.expiresAt).toLocaleString()}` : ""}</span>)}</div><small>Roles are selected by the user and activated by trial or payment status; administrators do not assign them.</small>{error && <p className="admin-error">{error}</p>}<div className="admin-actions"><button disabled={busy} onClick={() => save(status)}>{busy ? "Saving…" : user.status === "pending" ? "Activate account" : "Save account"}</button>{user.status !== "pending" && <button disabled={busy} onClick={() => save(status, true)}>Revoke sessions</button>}{user.status === "pending" && <button className="danger" disabled={busy} onClick={() => save("rejected")}>Reject</button>}</div></div>}
  </article>;
}
...
export default function AdminCenter({ initialData, principal }) {
  ...
  const [roleDraft, setRoleDraft] = useState({ name: "", description: "", isSelfSelectable: true, grants: [] });
  ...
  const createRole = async () => { setBusy(true); setRoleError(""); try { await request("/api/admin-center/roles", { method: "POST", body: JSON.stringify(roleDraft) }); setRoleDraft({ name: "", description: "", isSelfSelectable: true, grants: [] }); await refresh(); } catch (error) { setRoleError(error.message); } finally { setBusy(false); } };
  ...
  return <main className="admin-center-shell"><aside className="admin-sidebar"><a href="/apps" className="admin-brand"><span>AT</span>AgenticThat</a><p>Global Admin Center</p>{[["users", `Users${pending ? ` (${pending})` : ""}`], ["roles", "Roles & permissions"], ["workspaces", "Workspaces"], ["reviews", `Identity reviews${reviewCount ? ` (${reviewCount})` : ""}`], ["audit", "Audit history"]].map(([id, label]) => <button className={tab === id ? "active" : ""} onClick={() => setTab(id)} key={id}>{label}</button>)}<div className="admin-identity"><strong>{principal.name}</strong><small>{principal.email}</small></div></aside>
    <section className="admin-main"><header><p>Centralized access control</p><h1>{tab === "roles" ? "Roles and permissions" : tab.charAt(0).toUpperCase() + tab.slice(1)}</h1></header>
      ...
      {tab === "roles" && <div className="admin-role-layout"><section className="admin-panel"><h2>Create self-service role</h2><label>Role name<input value={roleDraft.name} onChange={(event) => setRoleDraft({ ...roleDraft, name: event.target.value })} /></label><label>Description<textarea value={roleDraft.description} onChange={(event) => setRoleDraft({ ...roleDraft, description: event.target.value })} /></label><label><input type="checkbox" checked={roleDraft.isSelfSelectable} onChange={(event) => setRoleDraft({ ...roleDraft, isSelfSelectable: event.target.checked })} /> Available for signup and payment</label><GrantMatrix value={roleDraft.grants} onChange={(grants) => setRoleDraft({ ...roleDraft, grants })} />{roleError && <p className="admin-error">{roleError}</p>}<button disabled={busy} onClick={createRole}>Create role</button></section><section className="admin-panel"><h2>Available roles</h2>{data.roles.map((role) => <RoleEditor role={role} onSaved={refresh} key={role.id} />)}</section></div>}
