...
test("trusted dashboard origins normalize paths to their exact origin", () => {
  assert.equal(
    helper.normalizeDashboardOrigin("https://demo.example.com/publishing?platform=instagram"),
...
test("trusted script IDs are stable across routes on the same dashboard", () => {
  assert.equal(
    helper.trustedScriptId("https://app.example.com/publishing"),
    helper.trustedScriptId("https://app.example.com/config-manager"),
  );
});
