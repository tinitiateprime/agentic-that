...
test("central publishing resumes reconnect-required jobs only after the paired Companion is online", () => {
  ...
  const companion = { id: "companion_1", status: "online", version: "2.1.19", runtimeStatus: "ready", lastSeenAt: new Date(timestamp).toISOString() };
...
test("central publishing reports account readiness and accepts only the active Companion job lease", () => {
  const timestamp = Date.now();
  const online = { status: "online", version: "2.1.19", runtimeStatus: "ready", lastSeenAt: new Date(timestamp).toISOString() };
...
test("central publishing enforces Companion compatibility and reports operational states", () => {
  const timestamp = Date.now();
  assert.equal(centralPublishingTestHelpers.versionAtLeast("2.1.19"), true);
  ...
  assert.equal(centralPublishingTestHelpers.companionStatus({
    status: "online", version: "2.1.19", runtimeStatus: "ready", updateStatus: "downloading",
    lastSeenAt: new Date(timestamp).toISOString(),
  }), "updating");
  assert.equal(centralPublishingTestHelpers.companionStatus({
    status: "online", version: "2.1.19", runtimeStatus: "error", lastSeenAt: new Date(timestamp).toISOString(),
  }), "error");
});
