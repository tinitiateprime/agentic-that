...
export async function supabasePublishingWorkspaceSnapshot(workspaceId) {
  ...
  const [row] = await sql`
    SELECT
      coalesce((SELECT value FROM public.job_control_settings WHERE key = 'minimum_companion_version'), '2.1.21') AS minimum_version,
  ...
  `;
  const companionValue = publicDevice(row?.companion, row?.minimum_version || "2.1.21");
