...
Most Watched ranks only reels whose exact view counts were captured from the fresh public Reels response and reconciled with Instagram's visible Reels-grid label. Conflicting counter fields are kept separate; the scraper does not assume that the largest field is the public view count. Compact or conflicting view labels may still be displayed as approximate values, but they are excluded from rankings, averages, and engagement calculations. `follower_count` is refreshed from the public profile payload with browser caching disabled, and `follower_count_display` preserves Instagram's visible profile label such as `23.9K`.
