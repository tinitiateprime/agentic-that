...
Profile Analysis scans up to 120 unique Reels by default before selecting ranking winners. Set `INSTAGRAM_MAX_REELS_SCAN` to a value from 50 through 300 to tune this bounded discovery depth. Scrolling tracks unique shortcodes rather than the current number of DOM nodes, so Instagram grid virtualization does not look like the end of the profile. Only likely ranking winners are opened for post-page likes, comments, captions, and timestamps.
