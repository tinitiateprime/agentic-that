...
For Profile and Profile URL jobs, the scraper reads the current visible profile and Reels grids and the anonymous public responses generated by those freshly opened pages. Latest prioritizes shortcodes discovered from current profile content, then uses public feed/bootstrap data only to enrich or recover missing fields. Range results and normally collected Latest results are accepted after the post page opens and its owner matches the requested username. If those post pages are temporarily unavailable, Latest may retain candidates tied directly to the requested profile's public Profile/Reels/feed response and marks the run partial.

Each job launches a fresh browser context, lets the launched Chromium report its own matching User-Agent, and requests revalidation instead of reusing browser, API, or CDN caches. Starting a job always attempts a new public scrape. If Instagram temporarily returns no usable public data, the API may return the most recent successful run for the same profile (six hours by default) and marks the run with `dataSource: "recent_cache"` and `discoveryStatus: "partial"`. Set `INSTAGRAM_CACHE_FALLBACK_MAX_AGE_MINUTES` to tune that bounded fallback window.
...
Latest profile collection uses a bounded candidate pool and paced post-page checks. When individual post pages are temporarily unavailable, candidates tied directly to the requested public Profile/Reels/feed response are retained as partial results instead of discarding the entire profile. Each run stores extraction diagnostics so navigation, ownership, range, and extraction failures can be distinguished in production.
