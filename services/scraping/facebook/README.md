...
The normalized post contract includes author, permalink, content, media type, thumbnail, timestamp, reactions, comment count and any publicly visible comment samples, Reels-grid views, followers, metric precision, current source, and capture time. Selected posts are opened through their public comment control, the comment region is scrolled when Facebook exposes one, and visible comments are collected. Share counts are intentionally not extracted. A value that is not reliably visible remains `null` and is shown as `N/A`.
